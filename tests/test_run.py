...
from qpbenchmark.worker import _solve_task
...
class TestProcessPoolParallelism(unittest.TestCase):
    ...
    def test_workers_run_in_parallel(self):
        ...
        worker_module = sys.modules["qpbenchmark.worker"]

        with patch.object(worker_module, "time_solve_problem", slow_solve):
...
class TestPerformance(unittest.TestCase):
    def test_parallel_speedup(self):
        ...
        # Patch time_solve_problem where it is imported in worker.py
        with patch(
            "qpbenchmark.worker.time_solve_problem", side_effect=slow_solve
        ):
